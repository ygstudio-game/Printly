...
router.post('/update-printers', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Optional: verify that the user owns the shop or has access
    if (shopId !== req.userShopId) {
 ...
 router.post('/settings', authMiddleware, async (req: AuthRequest, res: Response) => { 
  try {
    ...
    // Check user is only updating their own shop
...
router.post('/:shopId/printers', authMiddleware, async (req: AuthRequest, res: Response) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  ...
  try {
    ...
    // Authorization: only allow updating printers if requester's shop matches
    if (!req.userShopId || req.userShopId !== shopId) {
      return res.status(403).json({ error: 'Forbidden: You are not authorized to update printers for this shop' });
    }
...
router.get('/:shopId/jobs/history', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { shopId } = req.params;
...
router.get('/:shopId/jobs/pending', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { shopId } = req.params;
    const userShopId = req.userShopId;

    // Authorization: only allow if user's shop matches requested shopId
    if (!userShopId || userShopId !== shopId) {
      return res.status(403).json({ success: false, error: 'Forbidden: Access denied' });
    }
